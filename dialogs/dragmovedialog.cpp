...
#pragma region frameless

ResizeableHelper::ResizeableHelper(QWidget *target) :
    _target(target),
...
bool ResizeableHelper::eventFilter(QObject *o, QEvent*e) {
...
void ResizeableHelper::mouseRealese(QMouseEvent *e) {
    if (e->button() & Qt::LeftButton) {
        _leftButtonPressed = false;
        _dragStart = false;
    }
...
void ResizeableHelper::mouseMove(QMouseEvent *e) {
    if (_leftButtonPressed) {
        if (_dragStart) {
...
DragMoveDialog::DragMoveDialog(QWidget* parent):
QDialog(parent),
m_frameLess(nullptr)
{
...
DragMoveDialog::DragMoveDialog(bool resizeable, QWidget* parent):
QDialog(parent),
m_frameLess(new ResizeableHelper(this))
{
