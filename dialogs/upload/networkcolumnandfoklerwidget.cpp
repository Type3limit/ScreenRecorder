...
void NetworkColumnAndFoklerWidget::showEvent(QShowEvent* event)
{
	if (m_inited)
	{
		...
		{
			m_uploadOperator->getAllAuthorizedColumns(m_type == ColumnType_UploadMaterial,
				MaterialLibHelper::AddNleProject, false);
		}
		else if (m_type == ColumnType_CheckProject)
		{
			m_uploadOperator->getAllAuthorizedColumns(m_type == ColumnType_UploadMaterial,
...
void NetworkColumnAndFoklerWidget::initConnect()
{
	...
	auto onPersonalViewClick = [&](const QModelIndex& index)
		{
			auto personalSelection = ui->m_publicMaterialView->selectionModel();
			personalSelection->setCurrentIndex(index, QItemSelectionModel::SelectionFlag::ClearAndSelect);
	...
	auto onPublicMaterialViewClick = [&](const QModelIndex& index)
		{
			...
			auto publicSelection = ui->m_publicMaterialView->selectionModel();
			publicSelection->setCurrentIndex(index, QItemSelectionModel::SelectionFlag::ClearAndSelect);
			ui->m_filePathWidget->setDirs(QStringList() << index.data().toString());
			QMap<int, QVariant> map;
	...
	connect(ui->m_presonalMaterialButton, &QPushButton::clicked, [&, onPersonalViewClick](bool checked)
		{
			if (checked)
			{
				ui->m_presonalMaterialView->show();
				ui->m_publicMaterialButton->setChecked(false);
				ui->m_publicMaterialView->hide();
				onPersonalViewClick(m_presonalModel->firstIndex());
			}
			else
			{
				ui->m_presonalMaterialView->hide();
	...
	connect(ui->m_publicMaterialButton, &QPushButton::clicked, [&, onPublicMaterialViewClick](bool checked)
		{
			if (checked)
			{
				...
				ui->m_presonalMaterialView->hide();
				onPublicMaterialViewClick(m_publicModel->firstIndex());
			}
			else
			{
				ui->m_publicMaterialView->hide();
	...
	connect(ui->m_presonalMaterialView, &QTreeView::clicked, [&, onPersonalViewClick](const QModelIndex& index)
		{
			onPersonalViewClick(index);
		});

	connect(ui->m_publicMaterialView, &QTreeView::clicked, [&, onPublicMaterialViewClick](const QModelIndex& index)
		{
			onPublicMaterialViewClick(index);
		});
