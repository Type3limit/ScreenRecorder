...
{
    m_type = type;
    if(m_type == ColumnType_UploadMaterial) {//无分享收藏，只有文件夹
    ...
    }else if(m_type == ColumnType_UploadProject) {//无分享收藏，只有文件夹
...
{
    return ui->m_filePathWidget->data(ui->m_filePathWidget->dirs().size() - 1, Qt::UserRole + 1).toString();
}
...
void NetworkColumnAndFoklerWidget::showEvent(QShowEvent *event)
{
    if (m_inited)
    {
        ...
        if (m_type == ColumnType_UploadMaterial) {//无分享收藏，只有文件夹
            ...
            m_uploadOperator->getAllAuthorizedColumns(m_type == ColumnType_UploadMaterial, MaterialLibHelper::AddNleProject, true);
        }
        else if (m_type == ColumnType_CheckProject) {
            m_uploadOperator->getAllAuthorizedColumns(m_type == ColumnType_UploadMaterial, MaterialLibHelper::ViewNleProject, true);
        }
        else if (m_type == ColumnType_NewProject || m_type == ColumnType_NewProjectWithName) {
            m_uploadOperator->getAllAuthorizedColumns(m_type == ColumnType_UploadMaterial, MaterialLibHelper::AddNleProject, true);
        }
        else if (m_type == ColumnType_NewSeriesProject) {
            m_uploadOperator->getAllAuthorizedColumns(m_type == ColumnType_UploadMaterial, MaterialLibHelper::AddNleListProject, true);
        }
    }
}
...
//    ui->m_fileTreeView->setFocusPolicy(Qt::NoFocus);
    ...
    ui->m_typeComboBox->addItem(QStringLiteral("所有类型"), QVariant::fromValue(QList<MaterialLibHelper::MaterialType>() << MaterialLibHelper::project << MaterialLibHelper::folder));
    ui->m_typeComboBox->addItem(QStringLiteral("工程文件"), QVariant::fromValue(QList<MaterialLibHelper::MaterialType>() << MaterialLibHelper::project));
    ui->m_typeComboBox->addItem(QStringLiteral("文件夹"), QVariant::fromValue(QList<MaterialLibHelper::MaterialType>() << MaterialLibHelper::folder));
...
{
    connect(m_uploadOperator, &UploadOperator::publicColumns, [&](QList<QSharedPointer<SubRequests::ColumnBaseInfo>> &colList) {
    ...
    connect(ui->m_publicMaterialButton, &QPushButton::clicked, [&](bool checked) {
        if(checked) {
            if(m_type == ColumnType_CheckProject) {
                ...
                ui->m_filePathWidget->setDatas(QList<QMap<int, QVariant>>() << map);
                m_folderModel->clearData();
                m_uploadOperator->getFolder(false, "0", "0", ui->m_typeComboBox->currentData().value<QList<MaterialLibHelper::MaterialType>>(), ui->m_searchLineEdit->text());
            }
    ...
    connect(ui->m_presonalMaterialView, &QTreeView::clicked, [&](const QModelIndex &index) {
        ...
        ui->m_filePathWidget->setDatas(QList<QMap<int, QVariant>>() << map);
        m_folderModel->clearData();
        if(map[Qt::UserRole].toString() == "0") {//我的素材
            m_folderModel->onFolderSync(m_uploadOperator->getPersonalMediaSync(m_type == ColumnType_UploadMaterial, "0", ui->m_typeComboBox->currentData().value<QList<MaterialLibHelper::MaterialType>>(), ui->m_searchLineEdit->text()));
        }
    });
    connect(ui->m_publicMaterialView, &QTreeView::clicked, [&](const QModelIndex &index) {
        ...
        ui->m_filePathWidget->setDatas(QList<QMap<int, QVariant>>() << map);
        m_folderModel->clearData();
        if((nleEnabled && limit_folder == 0) || m_type == ColumnType_UploadMaterial || m_type == ColumnType_CheckProject) {//启用，且不限制才获取文件夹
            m_folderModel->onFolderSync(m_uploadOperator->getFolderSync(m_type == ColumnType_UploadMaterial, index.data(Qt::UserRole + 1).toString(), "0", ui->m_typeComboBox->currentData().value<QList<MaterialLibHelper::MaterialType>>(), ui->m_searchLineEdit->text()));
        }
    });
    ...
    connect(ui->m_fileTreeView->selectionModel(), &QItemSelectionModel::selectionChanged, this, &NetworkColumnAndFoklerWidget::selectionChanged);
    ...
    connect(m_folderModel, &UploadFolderModel::getMoreData, [&](int page, int size) {
        ...
        if(columnId == "0") {//我的素材
            m_folderModel->onFolderSync(m_uploadOperator->getPersonalMediaSync(m_type == ColumnType_UploadMaterial, folderId, ui->m_typeComboBox->currentData().value<QList<MaterialLibHelper::MaterialType>>(), ui->m_searchLineEdit->text(), page, size));
        }
...
//            m_folderModel->onFolderSync(m_uploadOperator->getPublicMaterialSync(MaterialLibHelper::ViewNleProject, ui->m_typeComboBox->currentData().value<QList<MaterialLibHelper::MaterialType>>(), ui->m_searchLineEdit->text(), page, size));
            m_folderModel->onFolderSync(m_uploadOperator->getFolderSync(m_type == ColumnType_UploadMaterial, "0", "0", ui->m_typeComboBox->currentData().value<QList<MaterialLibHelper::MaterialType>>(), ui->m_searchLineEdit->text(), page, size));
        }else {//公共栏目
            m_folderModel->onFolderSync(m_uploadOperator->getFolderSync(m_type == ColumnType_UploadMaterial, columnId, folderId, ui->m_typeComboBox->currentData().value<QList<MaterialLibHelper::MaterialType>>(), ui->m_searchLineEdit->text(), page, size));
        }
    });
}
...
{
    if(columnId == "0" || columnId == "1" || columnId == "2") {//我的素材
        ui->m_presonalMaterialButton->setChecked(true);
        ui->m_presonalMaterialView->show();
        ui->m_publicMaterialButton->setChecked(false);
        ui->m_publicMaterialView->hide();
