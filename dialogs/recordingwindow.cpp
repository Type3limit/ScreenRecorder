...
void RecordingWindow::init()
{
	...
	auto resetVideo = [&]()
		{
			...
			if (OBS_VIDEO_SUCCESS != m_obs->ResetVideo(cropRect.width() * devicePixelRatio,
				...
				cropRect.height() * devicePixelRatio, ui->frameRateComboBox->currentText().remove("FPS").toInt()))
			{
	...
	connect(ui->screenDeviceComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [&, resetVideo](int curIndex)
		{
...
void RecordingWindow::startRecord()
{
	if (!m_isRecordingStarted)
	{
		...
		m_obs->recOutAudio(m_isMicphoneEnable);
		m_obs->recSystemAudio(m_isPlayerEnable);
